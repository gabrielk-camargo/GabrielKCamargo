...
document.querySelector('form').addEventListener('submit', function(event) {
  ...
  .then(data => {
      ...
      if(data.success) {
          confirmationElement.innerText = 'Mensagem enviada com sucesso!';
          confirmationElement.style.display = 'block';
          confirmationElement.style.color = 'green';
      } else {
          confirmationElement.innerText = data.message || 'Ocorreu um erro ao enviar sua mensagem. Por favor, tente novamente mais tarde.';
          confirmationElement.style.display = 'block';
...
document.addEventListener("DOMContentLoaded", function() {
  ...
  const workBoxes = document.querySelectorAll(".col-md-4");
  ...
  filterButtons.forEach(button => {
    button.addEventListener("click", function() {
      ...
      workBoxes.forEach(box => {
        const boxClasses = box.className.split(" ");
        
        if (filterValue === "all" || boxClasses.includes(filterValue)) {
